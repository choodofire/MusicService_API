...
export class AlbumsService {
    ...
    async create(dto: CreateAlbumDto, image: any, userId: number): Promise<Album> {
        ...
        const album = await this.albumRepository.create({...dto, image: fileName, musicianId: musician.id})
        return album
    }
  ...
  async deleteAlbumById(value: number, userId: number): Promise<Album> {
      ...
      throw new HttpException("Ошибка при удалении альбома", HttpStatus.BAD_REQUEST);
