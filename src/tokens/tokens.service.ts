...
export class TokenService {
    ...
    async getToken(userId: number): Promise<string> {
        ...
        if (!tokens) throw new HttpException(
    ...
    async generateToken(user: GenerateTokensPayloadDto): Promise<GenerateTokensResponseDto> {
        const payload = { id: user.id, email: user.email, roles: user.roles, isActivated: user.isActivated  };
        return {
